...
def process_attempt(
    *,
    ...
    attempt_record = {
        ...
        "role_assignment_correct_count": comparison["role_assignment_correct_count"],
        "role_assignment_total_count": comparison["role_assignment_total_count"],
        "role_assignment_accuracy": comparison["role_assignment_accuracy"],
        "role_assignment_precision": comparison["role_assignment_precision"],
        "role_assignment_recall": comparison["role_assignment_recall"],
        "role_assignment_f1": comparison["role_assignment_f1"],
        "view_role_exact_match": comparison["view_role_exact_match"],
        "expected_top_view_slot": comparison["expected_top_view_slot"],
        "predicted_top_view_slot": comparison["predicted_top_view_slot"],
        "top_view_slot_match": comparison["top_view_slot_match"],
        "expected_side_view_slot": comparison["expected_side_view_slot"],
        "predicted_side_view_slot": comparison["predicted_side_view_slot"],
        "side_view_slot_match": comparison["side_view_slot_match"],
        "expected_end_view_slot": comparison["expected_end_view_slot"],
        "predicted_end_view_slot": comparison["predicted_end_view_slot"],
        "end_view_slot_match": comparison["end_view_slot_match"],
...
def summarize_attempts(attempts: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for (provider, case_id), rows in sorted(by_provider_case.items()):
        row = {
            ...
            "expected_top_view_slot": rows[0]["expected_top_view_slot"],
            "expected_side_view_slot": rows[0]["expected_side_view_slot"],
            "expected_end_view_slot": rows[0]["expected_end_view_slot"],
            "repeats": len(rows),
        }
        ...
        while True:
            ...
            try:
                ...
                print(
                    ...
                    f"view_role_exact={attempt_record['view_role_exact_match']} "
    ...
    attempts_csv_rows = [
        {
            ...
            "role_assignment_accuracy": item["role_assignment_accuracy"],
            "role_assignment_f1": item["role_assignment_f1"],
            "view_role_exact_match": item["view_role_exact_match"],
            "expected_top_view_slot": item["expected_top_view_slot"],
            "predicted_top_view_slot": item["predicted_top_view_slot"],
            "top_view_slot_match": item["top_view_slot_match"],
            "expected_side_view_slot": item["expected_side_view_slot"],
            "predicted_side_view_slot": item["predicted_side_view_slot"],
            "side_view_slot_match": item["side_view_slot_match"],
            "expected_end_view_slot": item["expected_end_view_slot"],
            "predicted_end_view_slot": item["predicted_end_view_slot"],
            "end_view_slot_match": item["end_view_slot_match"],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run view role classification benchmarks."
    )
...
def main() -> int:
    ...
    except (BenchmarkError, JsonParseError, ViewRoleSchemaError, ProviderError, ApiErrorGuardError, ValueError) as exc:
