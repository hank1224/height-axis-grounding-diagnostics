...
def synthetic_bbox_for_slot(slot: str) -> list[int]:
    return SLOT_SYNTHETIC_BBOXES[slot]
    ...
    for view in prediction.get("views", []):
        if isinstance(view, dict) and "bounding_box_2d" not in view:
            ...
            if slot in SLOT_SYNTHETIC_BBOXES:
                view["bounding_box_2d"] = synthetic_bbox_for_slot(slot)
    return prediction
...
def process_attempt(
    *,
    ...
    attempt_record = {
        ...
        "dimension_value_precision": comparison["dimension_value_precision"],
        "dimension_value_recall": comparison["dimension_value_recall"],
        "dimension_value_f1": comparison["dimension_value_f1"],
        "dimension_assignment_accuracy": comparison["dimension_assignment_accuracy"],
        "orientation_accuracy": comparison["orientation_accuracy"],
        "expected_dimension_count": comparison["expected_dimension_count"],
        "predicted_dimension_count": comparison["predicted_dimension_count"],
        "matched_dimension_count": comparison["matched_dimension_count"],
        ...
        while True:
            ...
            try:
                ...
                print(
                    ...
                    f"dimension_f1={attempt_record['dimension_value_f1']} "
    ...
    attempts_csv_rows = [
        {
            ...
            "dimension_value_f1": item["dimension_value_f1"],
            "dimension_assignment_accuracy": item["dimension_assignment_accuracy"],
            "orientation_accuracy": item["orientation_accuracy"],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run pure OCR image-level extraction benchmarks."
    )
...
def main() -> int:
    ...
    except (BenchmarkError, JsonParseError, OcrSchemaError, ProviderError, ApiErrorGuardError, ValueError) as exc:
