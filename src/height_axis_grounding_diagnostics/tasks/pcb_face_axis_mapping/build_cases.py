...
DEFAULT_PROMPT_PATH = "data/tasks/pcb_face_axis_mapping/prompt.md"
...
def make_ground_truth_template(cases: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "instructions": [
            ...
            "Fill `ground_truth` with the layout and PCB mounting face axis answer schema, excluding `bounding_box_2d`.",
            ...
            "For each occupied view, fill `pcb_mounting_face_axis` using one of +X, -X, +Y, -Y, +Z, -Z.",
            "The axis means the drawing-space direction from the component body toward the PCB mounting/connection face in that 2D view.",
            "Keep `ground_truth` as null while annotation_status is pending.",
            "Set annotation_status to complete after the layout and PCB face axis answer is fully reviewed.",
        ],
        "answer_groups": [
            {
...
def build(args: argparse.Namespace) -> None:
    ...
    ground_truth_path = args.output_dir / "ground_truth.json"
    template_payload = make_ground_truth_template(cases)
    ...
    if ground_truth_path.exists() and not args.force_ground_truth:
        print(f"Kept existing ground truth: {ground_truth_path.relative_to(ROOT)}")
    else:
        write_json(ground_truth_path, template_payload)
        print(f"Wrote ground truth: {ground_truth_path.relative_to(ROOT)}")

    print(f"Wrote {len(cases)} PCB face axis mapping cases: {cases_path.relative_to(ROOT)}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build image-level cases and a ground truth template for PCB face axis mapping."
    )
    ...
    parser.add_argument(
        "--force-ground-truth",
        action="store_true",
        help="Overwrite ground_truth.json if it already exists.",
    )
    return parser
