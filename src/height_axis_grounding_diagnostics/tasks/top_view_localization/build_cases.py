...
DEFAULT_PROMPT_PATH = "data/tasks/top_view_localization/prompt.md"
...
def make_ground_truth_template(cases: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "instructions": [
            ...
            "Fill `ground_truth` with the layout and top-view answer schema, excluding `bounding_box_2d`.",
            ...
            "Set `top_view_slot` to the occupied slot containing the package top view.",
            "Keep `ground_truth` as null while annotation_status is pending.",
            "Set annotation_status to complete after the layout and top-view answer is fully reviewed.",
        ],
        "answer_groups": [
            {
...
def build(args: argparse.Namespace) -> None:
    ...
    print(f"Wrote {len(cases)} top-view localization cases: {cases_path.relative_to(ROOT)}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build image-level cases and a ground truth template for top-view localization."
    )
